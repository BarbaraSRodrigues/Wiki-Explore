...
const SearchBar = ({ callback, updatePage, isResultPage }) => {
  ...
  const handleClick = async () => {
    ...
    // Verifica se a query está vazia antes de buscar
    if (query.trim() === '') {
      return;
    }

    setNumPage(updatePage);

    const URL = `http://localhost:8080/wikiexplorer/search?query=${encodeURIComponent(query)}&page=${numPage}`;
    console.log(URL);
  ...
  const classnameGroup = isResultPage ? 'group-search-bar-result' : 'group';
  const classnameInput = isResultPage ? 'input-wrapper-result' : 'input-wrapper';
  ...
  return (
    ...
    {/* Botões da SearchBar */}
    <div className={classnameInput} id="searchBar">
